...
local PredictionData = {LastPosition = nil, LastTick = 0, Velocity = Vector3zero}
...
local GetPredictedPosition = function(TargetPart)
	...
	if PredictionData.LastPosition then
		...
		if TimeDelta > 0 then
			PredictionData.Velocity = (CurrentPosition - PredictionData.LastPosition) / TimeDelta
		end
	end
	...
	local PredictedPosition = CurrentPosition + (PredictionData.Velocity * Settings.PredictionMultiplier)
			...
			if Environment.Locked then
				...
				if TargetPart then
					...
					if Settings.ShakeReduction then
						...
						local LerpedCFrame = CurrentCFrame:Lerp(TargetCFrame, Settings.ShakeReductionAmount)
					...
					if Environment.Settings.LockMode == 2 then
						...
						if Settings.Sensitivity > 0 then
							Animation = TweenService:Create(Camera, TweenInfonew(Environment.Settings.Sensitivity, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {CFrame = CFramenew(Camera.CFrame.Position, FinalPosition)})
							Animation:Play()
						else
							if Settings.AimSmoothing then
								local CurrentCFrame = Camera.CFrame
								local TargetCFrame = CFramenew(CurrentCFrame.Position, FinalPosition)
								__newindex(Camera, "CFrame", CurrentCFrame:Lerp(TargetCFrame, Settings.SmoothingFactor))
							else
